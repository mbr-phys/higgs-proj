...
\begin{itemize}
    \item Here are my plots for replicating what we did in first term:
        ...
        \end{figure}
    \item Like Oliver, the main difference seems to be with $b\to s\gamma$ where it fits higher than before -- \textbf{the combined fit yields $m_{H^+}>970\,$GeV at $2\sigma$ compared to $m_{H^+}\gtrsim\,$GeV in my previous fits}
    ...
    \item I have also included the fit for B mixing from higher-order expressions in \href{https://arxiv.org/pdf/1903.10440}{Crivellin 1903.10440} to show the difference -- I'm still looking into if there are errors causing the lines or if it's correct so it isn't included in the global fit yet
    \item Adding in $B_{s,d}\to\mu\mu$ and $\mathcal{R}(D^{(*)})$ yields similar results to what I had before:
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.45\textwidth]{rds_plot.png} \\
