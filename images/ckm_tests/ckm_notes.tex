...
\begin{itemize}
    ...
    \item For working in flavio, we need to write any modifications to SM formulae as NP WCs to be added to the SM WCs, so we need to rewrite this:
        ...
        \end{align*}
    \item Writing like this means we can just add on $\tVV-1$ to a WC (e.g. to \textbf{CVL\_bumunumu} in \href{https://wcxf.github.io/assets/pdf/WET.flavio.pdf}{flavio's WET basis}) and just multiply the WCs involved with $r_H$ by $\tVV$ 
    \item If the CKM element is not changed in the 2HDM, its contribution to the above will go away and we'll be back to our original consideration for the 2HDM; if it is changed, then we will be able to notice it
        \begin{itemize}
            \item \textit{NB - It may be possible to do something similar with semileptonic tree-levels without too much difficultly, I'm going to look into it}
        \end{itemize}
    ...
    \item I've done some quick contours for various values of $\tVV$ for the leptonic decays we've been using, which you can look through at \href{https://github.com/mbr-phys/higgs-proj/images/ckm_tests/}{https://github.com/mbr-phys/higgs-proj/images/ckm\_tests/} in individual folders for each CKM element consider
